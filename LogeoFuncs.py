...
def get_top_words_from_country(metric,country,n):
    ...
    title = "Top %n words (%metric) from".replace('%n',str(n))
    title = title.replace('%metric',metric)
    top500 = """<a href="/country?query=%query&metric=%metric&n=500">Top 500</a>""".replace('%query',country)
    top500 = top500.replace('%metric',metric)
    top1000 = """<a href="/country?query=%query&metric=%metric&n=1000">Top 1000</a>""".replace('%query',country)
    top1000 = top1000.replace('%metric',metric)
    top5000 = """<a href="/country?query=%query&metric=%metric&n=5000">Top 5000</a>""".replace('%query',country)
    top5000 = top5000.replace('%metric',metric)
    df_tfidfs = ''.join([title+' '+a.replace('_',' ')+'<div class="content1">'+b+"<br>"+top500+"<br>"+top1000+"<br>"+top5000+"</div>" 
                         for (a,b) in df_tfidfs])
    with open('templates/query_maps/cnt_'+metric+'_single_cnt_list.html', 'w', encoding='utf8') as f:
        f.write(df_tfidfs)
...
def make_country_map(country):
    ...
    #Change html stuff from generated folium map to suit the app's style. Find a better solution.
...
def strip_punct(line):
    ...
    for ch in line:
        charset.update(ch)
    punct = [ch for ch in list(set(charset)) if not ch.isalpha()]
    #print(punct)
    if ' ' in punct:
        punct.remove(' ')
    for ch in punct:
...
def remove_stops(line):
    line = line.split()
    return ' '.join([w for w in line if not w in stops and not w.isdigit()])
...
def get_cnt_sug(metric):
    ...
    df_tfidfs = ''.join(['<button class="collapsible">'+a.replace('_',' ')+"</button>"+'<div class="content">'+b
                         +'<br><a href="/country?query=%query&metric=%metric&n=200"><strong>See more</strong></a></div>'.replace('%query',a) for (a,b) in df_tfidfs])
